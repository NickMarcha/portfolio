...
| `portfolio-backend/stack.env` | Portainer env file. `CLOUDFLARE_TOKEN`, `ADMIN_PASSWORD` set in Portainer UI, not committed |
...
- **Visit tracking**: Frontend sends `POST /api/visits` with path on each page load (BaseHead). Backend stores in SQLite (`./data/visits.db`), enriches via ip-api.com. Docker volume `visit-data` persists data.
